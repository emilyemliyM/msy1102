...
from network.mynetwork import PPmodel_all_preprocess
...
def parse_args():
    ...
    parser.add_argument('--check_iter', type = int, default = 4)
...
def main():
    ...
    my_model = PPmodel_all_preprocess(grid_size = grid_size, max_pt_per_encode = 256,kernal_size = 1, fea_compre = compression_model)
    ...
    while (epoch < 2):
        ...
        for i_iter, data in enumerate(train_dataset_loader):
            ...
            train_pt_fea_ten_1 = [torch.from_numpy(i).type(torch.FloatTensor) for i in  train_pt_fea_1]
            # train_grid: [nponint,3]  ,trian_grid_ten: 不要z了
            train_grid_ten_1 = [torch.from_numpy(i[:, :2]) for i in train_grid_1]
            train_vox_ten_1 = [torch.from_numpy(i) for i in train_grid_1]
            point_label_tensor_1 = train_vox_label_1.type(torch.LongTensor)
            ...
            print( outputs_0.shape)  # [1 ,32, 480, 360]
            # loss_fun:是交叉熵
